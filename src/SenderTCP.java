		...
		public void run() {
			...
			while (true) {
				try {
					...
					String text = CommandExecutioner.sendQueue.poll();
					
					if (text != null) {
						byte[] data = text.getBytes("UTF-8");
						
						if (compressData) {data = RMSUtils.Compressor.Compress(data);}
						
						if (encryptData) {data = RMSUtils.AES.encrypt(data, encryptKey + "");}
